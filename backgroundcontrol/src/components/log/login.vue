...
<script>
  ...
  export default {
        ...
        this.Icode.toUpperCase()==this.code.toUpperCase()?(()=>{
          ...
          login(apple).then(res=>{
            res.code?this.$message.error(res.msg):(()=>{
              this.$cookies.set('remoteBackgroundToken',res.data.token);
    ...
    created() {
      //检查登录
      let token=this.$cookies.get('remoteBackgroundToken');
