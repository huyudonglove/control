<template>
    <div v-if="homePower.length">
      <el-card class="box-card" v-if="homePower[0].isCheck">
        <div slot="header" class="clearfix" style="height: 250px">
          ...
          <div style="float: left;height: 210px;padding: 20px;width: 30%">
            ...
            <h5>
              ...
              <span style="display: none">用户等级：{{msg.userGroupName}}</span>
            </h5>
            <h5>
          ...
          <div style="float: left;height: 210px;padding: 20px;text-align: right;width: 20%"><el-button :disabled="!homePower[1].isCheck" type="primary" @click="dialogVisible=true;">编辑资料</el-button></div>
        </div>
        <div  class="text item" style="display: flex;justify-content: space-between;padding: 20px">
          <div class="h-div" style="opacity: 0">
          ...
          <div class="h-div" style="opacity: 0"><h3>用户总数</h3><h4>{{count?count.userCount:0}}</h4></div>
        </div>
      </el-card>
...
export default {
  ...
  computed:{
    ...mapState('currentUserPower',{homePower:'homePower'}),
  },
  methods: {
    getSelfMsg () {
      getSelfMsg().then(v => {
