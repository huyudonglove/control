<template>
    <div style="padding: 50px">
      ...
      <div>
        <span class="myWords">设备编号：{{code}}</span>
        <span class="myWords">设备名称：{{equipmentName}}</span>
        <span class="myWords" style="margin-left: 10px;float: right">设备电量：{{power}}</span>
      </div>
      <div class="mainContainer" >
        <video id="videoElement" class="centeredVideo"  preload="none" muted ref="player" @click="getWH()">Your browser is too old which doesn't support HTML5 video.</video>
      </div>
      <div id="d1" style="display: none"></div>
      <div style="text-align: center;margin-top: 10px">
        <span v-if="controlAble"><span class="myWords">远程控制时长：</span><timerHu style="display: inline-block;margin-right: 10px"></timerHu></span>
...
<script>
  ...
  import timerHu from '../share/timer'
    export default {
        name: "vueFlv",
        components:{
          timerHu
        ...
        methods:{
                ...
                getAppReply(self.id).then(res=>{
                  console.log(res,77)
                        !res.code&&(()=>{
                          ...
                          res.data==2&&(()=>{
                            console.log(77777)
                            self.$message.warning(`用户已拒绝远程控制`)
                            self.end=false;
                            self.control=true;
          ...
          videoOn(){
            var dd=this.$refs.player;
            dd.oncanplay=alert("Can start playing video")
          }
        },
        mounted() {
